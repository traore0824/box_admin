<template>
  <div class="space-y-6 px-2 sm:px-0">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
      <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Retraits de Commission Utilisateur</h1>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden -mx-2 sm:mx-0">
      <div class="p-3 sm:p-4 border-b border-gray-200">
        <h2 class="text-sm sm:text-base md:text-lg font-semibold text-gray-800">Demandes de Retrait de Commission</h2>
      </div>
      ...
      <!-- Pagination -->
      <div v-if="store.totalWithdrawals > 0 && !store.isLoading" class="px-3 sm:px-4 py-3 bg-white border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-3">
      ...
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full">
        <div class="p-4 sm:p-6">
          ...
          <p class="text-sm text-gray-600 mb-4">
            Êtes-vous sûr de vouloir approuver cette demande de retrait de commission de
            <strong>{{ selectedWithdrawal ? formatCurrency(parseFloat(selectedWithdrawal.amount)) : '' }}</strong>
            pour l'utilisateur <strong>{{ selectedWithdrawal?.user_email }}</strong> ?
          </p>
          <p class="text-xs text-yellow-600 bg-yellow-50 p-2 rounded mb-4">
            <i class="fas fa-exclamation-triangle mr-1"></i>
            Attention : Cette action réinitialisera les compteurs de commission de l'utilisateur.
          </p>
    ...
    <div v-if="showRejectModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full">
        <div class="p-4 sm:p-6">
          ...
          <p class="text-sm text-gray-600 mb-4">
            Vous êtes sur le point de rejeter la demande de retrait de commission de
            <strong>{{ selectedWithdrawal ? formatCurrency(parseFloat(selectedWithdrawal.amount)) : '' }}</strong>
            pour l'utilisateur <strong>{{ selectedWithdrawal?.user_email }}</strong>.
          </p>
          <div class="mb-4">
...
import { useUserCommissionWithdrawalStore } from '../stores/userCommissionWithdrawal'
import { formatCurrency } from '../utils/currency'

const store = useUserCommissionWithdrawalStore()
...
const handleReject = async () => {
  ...
  try {
    await store.processWithdrawal(selectedWithdrawal.value.id, 'rejected', rejectionReason.value.trim())
